...
@TRANSFORMS.register_module()
class PackPoseInputsWithMask(PackPoseInputs):
    """Pack the inputs data for pose estimation, including the segmentation mask.

    This class extends the `PackPoseInputs` class to include the segmentation mask
    in the `PoseDataSample`.
    """
    ...
    def __init__(self,
                 meta_keys=('id', 'img_id', 'img_path', 'category_id',
                            ...
                            'raw_ann_info', 'dataset_name'),
                 pack_transformed=False):
        super().__init__(meta_keys=meta_keys, pack_transformed=pack_transformed)

    def transform(self, results: dict) -> dict:
        """Method to pack the input data, including the segmentation mask.
        ...
        Returns:
            ...
            - 'inputs' (obj:`torch.Tensor`): The forward data of models.
            - 'data_samples' (obj:`PoseDataSample`): The annotation info of the
                sample, including the segmentation mask.
        """
        # Call the parent class's transform method to pack the basic data
        packed_results = super().transform(results)

        # Get the data_sample from the packed results
        data_sample = packed_results['data_samples']

        # Add the segmentation mask to data_sample if it exists in results
        if 'gt_sem_seg' in results:
            mask = results['gt_sem_seg']
            if isinstance(mask, np.ndarray):
                # Convert the mask to a tensor if it is a numpy array
                mask = torch.from_numpy(mask).unsqueeze(0).long()  # Add channel dimension
                ...
                raise TypeError(
                    f"Expected mask to be a numpy array or torch.Tensor, but got {type(mask)}."
                )

            # Add the mask to data_sample as gt_sem_seg
            data_sample.gt_sem_seg = mask
