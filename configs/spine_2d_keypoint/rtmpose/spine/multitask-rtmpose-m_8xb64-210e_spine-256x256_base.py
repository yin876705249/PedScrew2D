...
# pipelines
train_pipeline = [
    dict(type='LoadImageAndMask', to_float32=True, mask_to_float32=False, mask_color_type='unchanged', apply_distance_transform=True),
    dict(type='GetBBoxCenterScale'),
    dict(type='RandomHalfBody'),
    dict(
        type='RandomBBoxTransform', scale_factor=[0.5, 1.5],
        rotate_factor=180),
    # dict(type='SpineRandomFlip', direction='horizontal'),
    dict(type='SpineTopdownAffine', input_size=codec['input_size']),
    dict(type='mmdet.YOLOXHSVRandomAug'),
    dict(
        type='SpineAlbumentation',
        transforms=[
            dict(type='Blur', p=0.1),
            dict(type='MedianBlur', blur_limit=5, p=0.1),
            dict(
                type='CoarseDropout',
                max_holes=1,
                max_height=0.4,
...
train_dataloader = dict(
    ...
    dataset=dict(
        ...
        data_root=data_root,
        data_mode=data_mode,
        # ann_file='annotations/fold_1/spine_2d_train.json',
...
val_dataloader = dict(
    ...
    dataset=dict(
        ...
        data_root=data_root,
        data_mode=data_mode,
        # ann_file='annotations/fold_1/spine_2d_val.json',
...
test_dataloader = dict(
    ...
    dataset=dict(
        ...
        data_root=data_root,
        data_mode=data_mode,
        # ann_file='annotations/fold_1/spine_2d_test.json',  
        # bbox_file=None,  # Provide if needed
        data_prefix=dict(img='images/', seg='masks/'),
        test_mode=True,
        pipeline=val_pipeline,
...
# hooks
default_hooks = dict(
    checkpoint=dict(save_best='OKS', rule='greater'))
